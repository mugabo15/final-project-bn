...
export class UsersService {
  ...
  async registerStudent(createStudentDto: CreateStudentDto) {
    try {
      const { regNumber, email } = createStudentDto;

      // Check for existing student with same regNumber OR email
      ...
      if (existingUser) {
        throw new BadRequestException('User with the provided registration number or email already exists.');
      }

      // Create new student
      const user = await this.userRepository.create({
        ...createStudentDto,
        facultyId: createStudentDto.schoolId,
        roles: 'student',
      } as User);
      ...
      if (this.mailService && email) {
        ...
        <div style="font-family: Arial, sans-serif; font-size: 15px; color: #333; line-height: 1.6;">
          <p>Dear ${createStudentDto.firstName || createStudentDto.lastName},</p>
          ...
          <p>If you encounter any issues or need support, feel free to reach out via the Help or Support section of the portal.</p>
  ...
  findAllStudents() {
    return this.userRepository.findAll({
      where: { roles: 'student' }, // use string not array
    });
  }
  ...
  findAllStaff() {
    return this.userRepository.findAll({
      ...
      include: [
        ...
        {
          model: this.sequelize.getRepository(Faculty),
          as: 'faculty',
          attributes: ['id', 'name'],
        },
        {
