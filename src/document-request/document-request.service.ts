...
export class DocumentRequestService {
  ...
  async createDocumentRequest(createDocumentRequestDto: CreateDocumentRequestDto, userId: number) {
    try {
      ...
      const documentRequestData = {
        ...
        documentType: createDocumentRequestDto.documentType,
        ...
        regNumber: userExist.dataValues.regNumber,
  ...
  async findAllDocumentRequests(query) {
  ...
  async approveDocumentRequest(id: number, role: string) {
    ...
    const roleStatusMap: Record<string, { approved: DocumentStatus; rejected: DocumentStatus }> = {
      ...
      dean: {
        approved: 'deanApproved',
        rejected: 'deanRejected',
      },
      registrationOfficer: {
      ...
      chancellor: {
        approved: 'chancellorApproved',
        rejected: 'chancellorRejected',
      },
    };
    ...
    // Update status only
    documentRequest.set('status', statusMap.approved);
  ...
  async updateRecomandationByStaff(
    id: number,
    updateRecomandationRequestStaffDto: UpdateRecomandationRequestStaffDto,
  ) {
  ...
  async rejectDocumentRequest(id: number, role: string, reason?: string) {
    ...
    if (reason) {
      documentRequest.set('reason', reason);
    }
  ...
  async deanUpdateTranscriptRequest(userId: number, updateDocumentRequestDto: HodUpdateTranscriptRequestDto) {
    ...
    // const faculty = await this.facultyRepository.findByPk(user.dataValues.facultyId);
    // if (!faculty) {
    //   throw new BadRequestException(`Faculty with ID #${transcriptRequest.facultyId} not found.`);
    // }
